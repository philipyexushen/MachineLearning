...
class DocumentClassifier:
    ...
    @staticmethod
    def __trainNB0(train_matrix:np.ndarray, train_category:np.ndarray):
        num_train_docs, num_words = np.shape(train_matrix)
        p_prior = np.sum(train_category) / float(num_train_docs)
        p0_num = np.ones(num_words); p1_num = np.ones(num_words)  # change to ones()

        # 拉普拉斯平滑，词只有出现和不出现两种情况
        p0_denominator = 2.0; p1_denominator = 2.0

        for i in range(num_train_docs):
            if train_category[i] == 1:
                p1_num += train_matrix[i]
                p1_denominator += np.sum(train_matrix[i])
            else:
                p0_num += train_matrix[i]
                p0_denominator += np.sum(train_matrix[i])
        p1_vec = np.log(p1_num / p1_denominator)
        p0_vec = np.log(p0_num / p0_denominator)
        # 得到各个词条件概率，以及先验概率
        return p0_vec, p1_vec, p_prior
